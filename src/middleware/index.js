...
exports.comparePass = async (request, response, next) => {
    try {
        ...
        // This pulls the user info from the database including thr hashed password
        console.log(request.user);
        const passCheck = await bcrypt.compare(request.body.password, request.user.password)
        console.log(passCheck);
        // This compares the unhashed password in the request body to the hashed password 
        // we stored in request.user
...
exports.tokenCheck = async (request, response, next) => {
    try {
        if (!request.header("Authorization")) {
            ...
            throw newError ("No token passed.");
        }
